...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module PSIAM.Utilities.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain PSIAM.Utilities.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "PSIAM.Utilities.psm1"
        } #validPSM1
